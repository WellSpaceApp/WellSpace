...
function doLogin(){
  ...
  if(authRole==='student'){
    ...
    CU={role:'student',...u};
    localStorage.setItem('ws_active_session', JSON.stringify({role:'student', email}));
    ...
    CU={role:'teacher',...u};
    localStorage.setItem('ws_active_session', JSON.stringify({role:'teacher', email}));
    toast(`Welcome back, ${u.name}! 📋`);
    loadTeacherDash();
  }
}
...
function logout(){
  ...
  localStorage.removeItem('ws_active_session');
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  // AUTO-LOGIN — restore session if user was already logged in
  try {
    const saved = localStorage.getItem('ws_active_session');
    if(saved){
      const session = JSON.parse(saved);
      if(session && session.role && session.email){
        if(session.role === 'student'){
          const u = gs().find(s => s.email === session.email);
          if(u){ CU={role:'student',...u}; loadStudentDash(); return; }
        } else if(session.role === 'teacher'){
          const u = gt().find(t => t.email === session.email);
          if(u){ CU={role:'teacher',...u}; loadTeacherDash(); return; }
        }
      }
      // Session was stale/invalid — clear it
      localStorage.removeItem('ws_active_session');
    }
  } catch(e){ localStorage.removeItem('ws_active_session'); }

  // No valid session — show entry screen
  showScreen('screen-entry');
});
